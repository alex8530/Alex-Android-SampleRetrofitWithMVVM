   ...
   companion object {
       ...
       fun showFailAlert(mActivity: Activity?,text: String ) {
           ...
           try {
               Alerter.create(mActivity)
                    .setText(text)
                   .setBackgroundColorInt(ContextCompat.getColor(mActivity, R.color.red))
       ...
       fun showWarinigAlert(mActivity: Activity?,text: String ) {
           ...
           try {
               Alerter.create(mActivity)
                   .setText(text)
                   .setBackgroundColorInt(ContextCompat.getColor(mActivity, R.color.yallow))
                   .enableInfiniteDuration(false)
                   .setDuration(3000)
                   .setContentGravity(Gravity.END)
                   .show()
       ...
       fun showSuccessAlert(mActivity: Activity?,text: String) {
           ...
           try {
               Alerter.create(mActivity)
                   .setText(text)
                   .setBackgroundColorInt(ContextCompat.getColor(mActivity, R.color.green))
                   .enableInfiniteDuration(false)
                   .setDuration(3000)
                   .setContentGravity(Gravity.END)
                   .show()
